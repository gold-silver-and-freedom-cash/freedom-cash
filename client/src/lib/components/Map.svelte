<script>
	...
	function mapAction(container) {
		...
		for (const poi of pois) {
			const newMarker = L.marker([poi.lat, poi.lon], { icon: markerIcon }).bindPopup(poi.text).addTo(map);
		...
		map.on('click', function (e) {
			...
			L.marker([newLocation.lat, newLocation.lon], { icon: markerIcon }).addTo(map);
		});
